...
namespace Syncfusion.EJ2.Base.PhysicalFileProvider
{

    public class PhysicalFileProvider : FileProviderBase
    {
        ...
        public virtual object GetFiles(string path, bool showHiddenItems, params object[] data)
        {
...
#endif
#if EJ2_DNX
        public virtual FileManagerResponse Upload(string path, IList<System.Web.HttpPostedFileBase> uploadFiles, string action, string[] replacedItemNames, params object[] data)
#else
...
#else
                                foreach (var filem in uploadFiles)
                                {
                                    var fileName = Path.GetFileName(file.FileName);
                                    file.SaveAs(Path.Combine(contentRootPath + path, fileName));
                                }
#endif
                            }
                            else
...
#if SyncfusionFramework4_0
        public virtual void Download(string path, string[] names, params object[] data)
        {
            try
            {
                String extension;
        ...
        //private void DownloadFolder(string path, string[] names, int count)
        //{
        //    {
        //        HttpResponse response = HttpContext.Current.Response;
        ...
        //                    for (var i = 0; i < names.Length; i++)
        //                    {
        //                        currentDirectory = Path.Combine(this.contentRootPath, names[i]);
        ...
        //                        }
        //                        else
        //                            zipEntry = archive.CreateEntryFromFile(currentDirectory, names[i]);
        //                    }
        //                }
        //            }
        //        }
        ...
        //            response.Buffer = true;
        //            if (names.Length == 1)
        //                response.AddHeader("content-disposition", string.Format("attachment; filename={0}", names[0] + ".zip"));
        //            else
        //                response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "folders.zip"));
        ...
        public virtual void DownloadFile(string path, string[] names = null)
        {
        ...
        FileStreamResult FileProviderBase.Download(string path, string[] names, params object[] data)
        {
